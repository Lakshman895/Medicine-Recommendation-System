...
class DataIngestion:
    ...
    def data_ingestion(self):
        """Ingestion data form the configured URLs and saves it as CSV files."""
        
        self.creating_directories()
        self.url_lists = self.config.source_url
        self.file_name_list = ['training', 'description', 'diets', 'medications', 'precautions', 'workout']
        
        try:
            for url,name in zip(self.url_lists,self.file_name_list):
                logging.info(f'Retrieving data form {url}')
                response = requests.get(url)
                json_data = response.json()
                data = pd.DataFrame(json_data)
                logging.info('Loaded data sucessfully from url: {url}')
